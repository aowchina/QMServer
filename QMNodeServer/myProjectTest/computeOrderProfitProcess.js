...
var computeProfitServer = require('./computeOrderProfit');
...
function initConf() {
    ...
    flags.defineString('name', 'computeProfitServer', 'server name');
...
function initialize(callback) {
    server = new computeProfitServer.ComputeProfitServer(serverConfig);
    server.listen(function(err) {
        if (err) {
            logger.info('computeProfitServer listen err', err);
            process.exit(1);
        }

        logger.info('computeProfitServer listen ok');
        callback(err);
    });
    global.ComputeProfitServer = server;
}
