...
class TehuiModelTehui extends JModelAdmin{
	...
	public function save($data){
		...
		if (empty($val_desc)) {
			$this->setError("：请填写价格说明");
			return false;
		}

		if (!preg_match('/^[\x{4e00}-\x{9fa5}|a-zA-Z0-9\-\_\s\(\)\（\）\,\，\.\。]{2,50}$/u', $val_desc)) {
			$this->setError("：价格说明在50个字以内，不得含有非法字符");
			return false;
		}
